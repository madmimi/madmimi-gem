...
class TestMadmimi < Test::Unit::TestCase
  context "An API call" do
    ...
    should "get a suppressed status" do
      stub_get("/audience_members/#{URI.escape('dave@example.com')}/is_suppressed", { :https => true, :body => "sent" })
      response = @mimi.suppressed?('dave@example.com')
      assert_equal false, response
    end

    should "suppress email" do
      stub_post("/audience_members/#{URI.escape('dave@example.com')}/suppress_email", { :https => true, :body => "sent" })
    ...
    should "unsuppress email" do
      stub_post("/audience_members/#{URI.escape('dave@example.com')}/unsuppress_email", { :https => true, :body => "sent" })
